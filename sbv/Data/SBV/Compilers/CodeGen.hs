...
module Data.SBV.Compilers.CodeGen (
        ...
        , cgInput, cgInput', cgInputArr
        , cgOutput, cgOutput', cgOutputArr
...
import Control.Monad.State.Lazy  (MonadState, StateT(..), modify', get)
...
-- | Creates an atomic input in the generated code.
cgInput' :: SymVal a => SBVCodeGen (SBV a)
cgInput' = do
  r <- forall_
  sv <- sbvToSymSV r
  modify' $ \s ->
    let nm = "v" ++ show (cgUniq s)
        uniq = cgUniq s + 1
    in  s { cgInputs = (nm, CgAtomic sv) : cgInputs s, cgUniq = uniq }
  return r
...
-- | Creates an atomic output in the generated code.
cgOutput' :: SBV a -> SBVCodeGen ()
cgOutput' v = do _ <- output v
                 sv <- sbvToSymSV v
                 modify' $ \s ->
                   let nm = "v" ++ show (cgUniq s)
                       uniq = cgUniq s + 1
                   in s { cgOutputs = (nm, CgAtomic sv) : cgOutputs s, cgUniq = uniq }
...
instance Annotated SrcInfo SBVCodeGen (SBV a) where
  ...
  annotateM ma (Info (Just nm) _) = do
    ...
    return sbv
      where update og_s s
              | length (cgInputs s) > length (cgInputs og_s) =
                let (_ , x) = head (cgInputs s)
...
instance Annotated SrcInfo SBVCodeGen () where
  ...
  annotateM ma (Info (Just nm) _) = do
    ...
    return sbv
      where update og_s s
              | length (cgOutputs s) > length (cgOutputs og_s) =
