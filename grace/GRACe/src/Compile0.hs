...
translateGCMCommand = \case
  Output (Port (v, Just str)) _ -> do
    let i = varID v
    modify $ \st -> st { outputs = outputs st ++ ["\\\"" ++ str ++ "\\\"" ++ " : \\(v" ++ show i ++ ")"]}
  Output (Port (v, _)) (Just str) -> do
    let i = varID v
    modify $ \st -> st { outputs = outputs st ++ ["\\\"" ++ str ++ "\\\"" ++ " : \\(v" ++ show i ++ ")"]}
  Output (Port (v, _)) _ -> do
    let i = varID v
    modify $ \st -> st { outputs = outputs st ++ ["\\\"" ++ "output" ++ show i ++ "\\\"" ++ " : \\(v" ++ show i ++ ")"]}
  ...
  CreateAction p@(Param a (Port (j, _))) -> do
    vid <- gets nextVarId
