...
translateActionCommands' (Act expr (Action _ (Param _ (Port (j, _))))) = do
...
getAndIncrPort :: (MonadState CompilationState m) => m Int
getAndIncrPort = do
  x <- gets nextPortId
  modify $ \st -> st { nextPortId = x + 1 }
  return x

translateGCMCommand' :: GCMCommand a -> IntermMonad a
translateGCMCommand' = \case

  Output (Port (v, Just str)) _ -> do
    let i = varID v
    modify $ \st -> st { outputs = outputs st ++ [(str, varNameStr i)] }
  Output (Port (v, Nothing)) (Just str) -> do
    let i = varID v
    modify $ \st -> st { outputs = outputs st ++ [(str, varNameStr i)] }
  Output (Port (v, _)) _ -> do
    let i = varID v
    modify $ \st -> st { outputs = outputs st ++ [("output" ++ show i, varNameStr i)] }
  ...
  CreatePort proxy -> do
    ...
    let line = declVar varType (varNameStr vid)
    modify $ \st -> st { model = model st ++ [line] }
    return $ Port (Var vid, Just ("port" ++ show pid))
  ...
  CreateParam proxy def -> do
    ...
    let lines = [ declVar varType (varNameStr vid)
                ...
                , constr (HZBuiltIns.not_ (HZ.Var (actName vid)) HZBuiltIns.->.
                           (HZ.Var (varName vid) HZBuiltIns.=.= hzConst def))
                ]
    modify $ \st -> st { model = model st ++ lines
                       }
    return $ Param def (Port (Var vid, Nothing))
  CreateAction p@(Param _ (Port (j, _))) -> do
    vid <- getAndIncrVar
    let varType = HZ.Int
    let lines = [ declVar varType (varNameStr vid)
...
compileGCM' gcm = stateToModel $ flip execState (CompilationState [] [] [] 0 0) $ interpret gcm
  where
    stateToModel cs =
      model cs ++
