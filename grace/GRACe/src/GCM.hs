...
data GCMCommand a where
  Output         ::             Port a     -> Maybe String -> GCMCommand ()
...
output p s = Instr (Output p (Just s))

output' :: Port a -> GCM ()
output' p = Instr (Output p Nothing)
...
taken (Action i _) = return (Port (Var i, Nothing))
